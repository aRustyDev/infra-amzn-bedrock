    ...
    Statement = [
      {
        ...
        Resource = [
          "arn:aws:bedrock:${var.region}::foundation-model/${local.claude_sonnet_4_model_id}",
          "arn:aws:bedrock:${var.region}::foundation-model/${local.regional_claude_sonnet_4_model_id}"
        ]
        Condition = {
          StringEquals = {
            "aws:PrincipalArn" = [
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          "arn:aws:logs:${var.region}:${var.account_id}:log-group:/aws/bedrock/*"
        ]
      },
      {
        Sid    = "IAMPassRole"
